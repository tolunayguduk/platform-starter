...
public interface RolePermissionRepository extends JpaRepository<RolePermission, Long> {
    ...
    @Query("select rp from RolePermission rp " +
           ...
           "where rp.role.name in :roleNames and rp.accessLevel = com.platform.user.authz.RolePermission.AccessLevel.GRANTED")
    List<RolePermission> findGrantedByRoleNames(Collection<String> roleNames);

    @Query("select rp from RolePermission rp " +
           "join fetch rp.permission " +
           "where rp.role.name in :roleNames and rp.accessLevel = com.platform.user.authz.RolePermission.AccessLevel.VISIBLE_DENIED")
